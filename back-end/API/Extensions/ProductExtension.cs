...
namespace API.Extensions
{
    public static class ProductExtension
    {
        ...
        public static IQueryable<Product> Search(this IQueryable<Product> query, string searchTerm)
        {
            ...
            query = query.Where(p => p.Name.ToLower().Contains(lowerSerchTerm));
        ...
        public static IQueryable<Product> Filter(this IQueryable<Product> query, string brand, string type)
        {
            ...
            query = query.Where(p => brandList.Count == 0 || brandList.Contains(p.Brand.ToLower())); // if == 0 do notthing, else do the second stament
